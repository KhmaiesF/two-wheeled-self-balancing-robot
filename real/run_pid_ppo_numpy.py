...
PRÉREQUIS:
    pip install numpy
    Poids policy NPZ dans: models/ppo_policy_weights.npz

UTILISATION:
    python3 run_pid_ppo_numpy.py              # Mode PID+PPO (NumPy)
    python3 run_pid_ppo_numpy.py --pid_only   # Mode PID seul (fallback)
    python3 run_pid_ppo_numpy.py --verbose    # Affichage debug
================================================================================
"""
...
def load_ppo_model(weights_path: str = "models/ppo_policy_weights.npz"):
    """Charge les poids PPO exportés pour inference NumPy."""
    model_path = Path(weights_path)
    if not model_path.exists():
        print(f"[WARN] Poids PPO NumPy non trouvés: {model_path}")
        return None

    try:
        model = PpoNumpyPolicy(str(model_path))
        print(f"[OK] Policy NumPy chargée: {model_path} (input_dim={model.input_dim})")
        return model
...
def main():
    ...
    parser.add_argument("--ppo-weights", type=str, default="models/ppo_policy_weights.npz",
                        help="Chemin des poids PPO NumPy (defaut: models/ppo_policy_weights.npz)")
    ...
    if not args.pid_only:
        ppo_model = load_ppo_model(args.ppo_weights)
