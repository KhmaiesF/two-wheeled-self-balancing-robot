...
def evaluate_scenario(
    ...
    for ep in range(n_episodes):
        ...
        while not done:
            ...
            max_pitch = max(max_pitch, abs(obs[0]))
            max_wheel_speed = max(max_wheel_speed, abs(obs[4]), abs(obs[5]))
    ...
    avg_max_pitch = np.mean([r["max_pitch_deg"] for r in results])
    avg_max_wheel = np.mean([r["max_wheel_speed"] for r in results])
    ...
    return {
        ...
        "avg_steps": avg_steps,
        "avg_max_pitch_deg": avg_max_pitch,
        "avg_max_wheel_speed": avg_max_wheel,
    }
...
def main():
    ...
    for scenario_name in scenarios:
        ...
        print(f"  Pitch max moyen: {result['avg_max_pitch_deg']:.1f}°")
        print(f"  Vitesse roue max: {result['avg_max_wheel_speed']:.1f} rad/s")
    
    # Résumé
    print("\n" + "=" * 60)
    print("📋 RÉSUMÉ")
    print("=" * 60)
    print(f"{'Scénario':<15} {'Succès':>10} {'Reward':>10} {'Pitch max':>12}")
    print("-" * 60)
    for r in results:
        print(f"{r['scenario']:<15} {r['success_rate']*100:>9.0f}% {r['avg_reward']:>10.1f} {r['avg_max_pitch_deg']:>11.1f}°")
